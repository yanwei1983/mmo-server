...
void Matrix3::GolubKahanStep(Matrix3& kA, Matrix3& kL, Matrix3& kR)
{
    ...
    // adjust right
    float fY         = kA[0][0] - (Math::Abs(fRoot1 - fT22) <= Math::Abs(fRoot2 - fT22) ? fRoot1 : fRoot2);
    float fZ         = kA[0][1];
...
//-----------------------------------------------------------------------
void Matrix3::FromEulerAnglesXZY(const Radian& fYAngle, const Radian& fPAngle, const Radian& fRAngle)
{
    float fCos, fSin;
